...
module WillSupport
  module Selenium
    class Page
      ...
      def selector(name)
        self.class.selectors.fetch(name) { fail KeyError, "undefined selector #{name} for page #{page_name}"}
      end
      ...
      module ClassMethods
        ...
        def element(name, *args, &block)
          fail "an element named #{name} is already defined" if selectors.key?(name)
